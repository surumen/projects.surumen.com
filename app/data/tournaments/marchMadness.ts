...
/** Initial placeholders for the Final region champions */
const finalSeeds: Record<string, SeedMeta> = Object.fromEntries(
    Object.keys(regionSeeds).map(region => [region, { name: '' }])
...
/** Build each region (R64 → R32 → S16 → E8) */
const regions = Object.fromEntries(
    Object.entries(regionSeeds).map(([region, seedsMap]) => [
        region,
        buildCustomRegion(region, seedsMap, ncaaTeams, initialR64),
    ])
);
...
const finalGames: GameData[] = [
    {
        ...
        gameNumber: 0,
        region: 'Final',
        sourceGame1: fromGame('West', 3, 0),  // E8 winner of West
