                    ...
                    <Col xs={12} md={12/regionsPerRow} className="px-0 h-100" key={r}>
                        <Region
                            ...
                            onAdvanceTeam={(game,round,gameIdx,seed)=>
                                dispatch(advanceTeam({tournamentKey:keyString,region:r,game,round,gameIdx,seed}))
                            }
                        />
                    </Col>
                )
            ...
            {!isMobile && (
                <div
                    style={{
                        position: 'absolute',
                        top: 0,
                        left: `${(100/regionsPerRow)/2}%`, // centers the block horizontally
                        width: `${100/regionsPerRow}%`,
                        height: '100%',                     // <-- fill full height now
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        pointerEvents: 'auto',              // <-- allow clicks again
                        zIndex: 1,                          // keep it above the other Regions
                    }}
                >
                    <Region
                        name="Final"
                        ...
                        isFinal
                        onAdvanceTeam={(game,round,gameIdx,seed)=>
                            dispatch(advanceTeam({tournamentKey:keyString,region:'Final',game,round,gameIdx,seed}))
                        }
                    />
                </div>
            )}
