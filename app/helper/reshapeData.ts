...
*/
export const groupDataByFirstColumn = (data: any[], columnIndex: number) => {
    const column_names = new Set(data.map(x => x[Object.keys(x)[columnIndex]]));
...
export const reshapeData = (data: any[], groupByIndex: number, nameIndex: number, valueIndex: number) => {
    ...
    const groupedByColumn = _.groupBy(data, (e) => e[Object.keys(e)[groupByIndex]]);
